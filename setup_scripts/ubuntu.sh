...
if [ $(echo `pwd` | grep setup_scripts | wc -l) -ne 0 ]; then
	...
	# place spigot alongside cryptoplugin
	mkdir -p ../spigot
	cd ../spigot
	spigot_dir=`pwd`
	rm $spigot_file 2> /dev/null
	wget http://jenkins.cryptoplugin.co/job/spigot/lastSuccessfulBuild/artifact/Spigot/Spigot-Server/target/$spigot_file
	...
	echo 'Creating the plugins directory and linking the cryptoplugin plugin'
	mkdir -p plugins
	rm plugins/cryptoplugin-all.jar 2> /dev/null
	ln -s $cryptoplugin_dir/build/libs/cryptoplugin-all.jar $spigot_dir/plugins/cryptoplugin-all.jar
